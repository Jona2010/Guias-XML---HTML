...
function obtenerFiltros() {
  return {
    ...
    origen: document.querySelector('input[placeholder="Origen"]').value.toLowerCase(),
    destino: document.querySelector('input[placeholder="Destino"]').value.toLowerCase()
  };
}
...
function cargarFiltros() {
  ...
  const anios = [...new Set(historial.map(g => new Date(g.fecha).getFullYear()))];
...
function filtrarHistorial() {
  ...
  const resultado = historial.filter(g => {
    ...
    return (
      (!filtros.texto || g.numero.toLowerCase().includes(filtros.texto)) &&
      (!filtros.anio || anio === filtros.anio) &&
      (!filtros.mes || mes === filtros.mes) &&
      (!filtros.origen || g.origen.toLowerCase().includes(filtros.origen)) &&
      (!filtros.destino || g.destino.toLowerCase().includes(filtros.destino))
    );
  });

  renderHistorial(resultado);
}
...
function renderHistorial(lista) {
  ...
  lista.forEach(g => {
    ...
    div.innerHTML = `
      <strong>${g.numero}</strong><br>
      <small>${g.fecha}</small><br>
      <span>${g.origen} → ${g.destino}</span>
    `;

    div.onclick = () => cargarGuia(g);
